...
function getCommentData(comment, eventId) {
	return {
		...
		eventId: eventId,
		bodyHtml: cheerio.load(comment.content)('p').text().trim(),
	...
	if(item.article.url.indexOf('/liveblogs/') === -1 && item.article.url.indexOf('/marketslive/') === -1) {
		...
		return suds(item.article.articleId, item.article.url)
			.then(res => {
				...
				let logMsg = isNewArticle ? 'inserted' : 'updated';
				log.info({commentId: comment.commentId, article: {articleId: article.id, url: article.url}}, logMsg);
				if(isNewArticle) {
					actions.push(
